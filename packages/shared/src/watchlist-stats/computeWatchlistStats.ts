...
export function computeWatchlistStats(
  input: ComputeWatchlistStatsInput,
): WatchlistStatsCache {
  const referenceDate = input.referenceDate ?? new Date();
  const challenge = input.challenge;
  const tvFiltered = filterMembershipsByTvRule(
    input.memberships,
    challenge?.tvCountRule,
  );
  ...
  const percentComplete =
    totalEligible > 0
      ? Math.min(100, Math.round((completed / totalEligible) * 100))
      : 0;
...
function isActiveMembership(membership: StatsMembershipInput): boolean {
  return (
    membership.listStatus !== "skipped" && membership.countsTowardGoal !== false
  );
}
