...
- Extend [`MediaDisplay`](apps/plotline/src/features/media/types.ts) with optional `voteAverage?: number`
- Add optional prop or derive from `media.voteAverage` in [`MediaItem.tsx`](apps/plotline/src/features/media/components/MediaItem.tsx)
- Render rating `Badge` as absolutely positioned overlay on poster wrapper in `MediaPoster` or `ItemHeader` (prefer `MediaPoster` with optional `rating` slot/prop to keep overlay scoped to poster)
- Keep existing `addToList` prop optional — omit on search grid unless add-to-list UX is added in same phase
...
- `src/features/search/components/SearchResultGrid.tsx` — maps TMDB results to `MediaCard`
- `src/features/media/components/MediaItem.tsx` — augment `MediaCard` + optional rating badge on poster
- `src/features/media/types.ts` — add `voteAverage` to `MediaDisplay`
- `src/components/utils/MediaPoster.tsx` — optional rating overlay prop (if badge lives on poster)
...
- Reuse `MediaCard` / `LibraryItemRow`, `StatusBadge`, `EmptyState`
